        ...
        <div class="header">
            <ul ref="headerul">
                <li class="header_li_one" @click="openlist">
                    <h2>{{text}}</h2>
                    <p>
                        <span class="iconfont icon-xiangxia" ref="btn" style="display:inline-block;transition: all 0.5s"></span>
                    </p>
                </li>
                <ul v-show="show">
                    <li v-for="item in option1" :key="item.value" class="list" @click="listswith(item.value)"  :class="{active: item.value === num}" >
                        {{item.text}}
                    </li>
                </ul>
